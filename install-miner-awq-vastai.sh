...
# MIA GPU Miner Installer - AWQ Version for Vast.ai
# Optimized for Vast.ai containers
...
echo -e "${GREEN}╔═══════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║   MIA GPU Miner - AWQ (Vast.ai Edition)   ║${NC}"
...
# Skip GPU detection - assume it exists on Vast.ai
echo -e "${YELLOW}Running on Vast.ai - GPU assumed to be available${NC}"
echo ""
...
# Install PyTorch with CUDA support (Vast.ai usually has CUDA 11.8)
...
#!/usr/bin/env python3
"""
MIA Unified GPU Miner - AWQ Version (Vast.ai)
"""
import os
...
class MinerClient:
    ...
    def __init__(self, model_server):
        ...
        self.miner_name = f"gpu-miner-{socket.gethostname()}-vastai"
        self.miner_id = None
    ...
    def get_gpu_info(self):
        """Get GPU information"""
        try:
            # Use PyTorch to get GPU info on Vast.ai
            if torch.cuda.is_available():
                return {
            ...
            pass
        return {'name': 'Vast.ai GPU', 'memory_mb': 8192}  # Default assumption
...
def main():
    """Main entry point"""
    logger.info("=" * 60)
    logger.info("MIA Unified GPU Miner - AWQ Version (Vast.ai)")
    logger.info("=" * 60)
...
# Create a start script for Vast.ai
cat > start_on_vastai.sh << 'EOF'
#!/bin/bash
...
export PATH="/usr/local/cuda/bin:$PATH"
export LD_LIBRARY_PATH="/usr/local/cuda/lib64:$LD_LIBRARY_PATH"
nohup python mia_miner_unified.py > miner.log 2>&1 &
echo "Miner started! Check miner.log for output"
EOF
chmod +x start_on_vastai.sh

echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}✓ MIA GPU Miner (AWQ) installed for Vast.ai!${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════════${NC}"
echo ""
...
echo "  ./run_miner.sh"
echo ""
echo "Option 2 - Run in background (recommended for Vast.ai):"
echo "  cd $INSTALL_DIR"
echo "  ./start_on_vastai.sh"
echo ""
echo "Option 3 - View logs:"
echo "  cd $INSTALL_DIR"
echo "  tail -f miner.log"
echo ""
echo -e "${YELLOW}Vast.ai specific notes:${NC}"
echo "• The miner will auto-start if you added ./start_on_vastai.sh to your docker startup"
echo "• Check the Vast.ai console logs if you have issues"
echo "• Make sure port 8000 is not exposed publicly"
echo ""
echo -e "${GREEN}Your miner is now mining for MIA!${NC}"