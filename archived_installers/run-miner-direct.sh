...
try:
    ...
    tokenizer = AutoTokenizer.from_pretrained('NousResearch/Nous-Hermes-2-Mistral-7B-DPO', local_files_only=True)
    print('✓ Model already downloaded')
except:
    print('Model not found, downloading now...')
    print('This is a 14GB download and will take 10-20 minutes.')
    from transformers import AutoTokenizer, AutoModelForCausalLM
    AutoTokenizer.from_pretrained('NousResearch/Nous-Hermes-2-Mistral-7B-DPO')
    AutoModelForCausalLM.from_pretrained('NousResearch/Nous-Hermes-2-Mistral-7B-DPO', torch_dtype='auto', low_cpu_mem_usage=True)
