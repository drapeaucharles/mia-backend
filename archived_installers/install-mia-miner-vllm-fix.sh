...
# MIA GPU Miner - Fixed for vLLM puccinialin error
...
# Install vLLM dependencies manually to avoid puccinialin issue
echo -e "${YELLOW}Installing vLLM dependencies...${NC}"
...
# Try installing vLLM with no-deps first, then install its real dependencies
echo -e "${YELLOW}Installing vLLM (working around puccinialin issue)...${NC}"
pip install vllm --no-deps || {
    echo -e "${YELLOW}Trying alternative vLLM installation method...${NC}"
    # If that fails, try installing specific version
    pip install vllm==0.2.7 --no-deps || {
        echo -e "${RED}vLLM installation failed. Trying text-generation-inference instead...${NC}"
