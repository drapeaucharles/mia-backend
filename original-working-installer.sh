#!/bin/bash
# Original Working Miner Installer - Exact recreation

echo "🚀 Installing Original Working Miner"
echo "===================================="
...
apt-get update -qq
apt-get install -y python3.11 python3.11-venv python3.11-dev git wget curl
...
# Create requirements.txt (exact versions from working setup)
...
pip install transformers==4.35.0 accelerate==0.24.1 sentencepiece==0.1.99 protobuf==3.20.3
pip install vllm==0.2.7
pip install flask==3.0.0 waitress==2.1.2 requests==2.31.0
...
def extract_tool_call(response: str) -> Optional[Dict]:
    ...
    # Look for tool call in XML tags
...
def generate():
    """Generate endpoint compatible with MIA backend"""
    try:
        data = request.json
...
echo ""
echo "✅ Original Working Miner Installed!"
echo "===================================="
echo ""
echo "Installed at: /data/qwen-awq-miner"
echo "Using: Qwen2.5-32B-Instruct-AWQ with vLLM"
echo ""
echo "To start:"
